...
Scrapes project information from https://meguru-construction.com/works/
"""
...
class MeguruScraper:
    ...
    def get_all_project_links(self):
        """Get all project links from all pages (max 4 pages)"""
        all_links = []
        max_pages = 4
    ...
    def is_valid_project_data(self, project_data):
        """Check if project data has meaningful content"""
        if not project_data:
            return False
    ...
    def scrape_all_projects(self):
        ...
        print("Starting to scrape Meguru Construction projects...")
        ...
        for i, link in enumerate(project_links, 1):
            ...
            if self.is_valid_project_data(project_data):
                self.projects.append(project_data)
            else:
                print(f"Skipping invalid project data for: {link}")
            time.sleep(1)  # Be respectful to the server
        
        print(f"Successfully scraped {len(self.projects)} projects")
        return self.projects

    def save_to_json(self, filename="output/meguru_projects.json"):
